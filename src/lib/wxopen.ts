 ...
 * 获取授权方接口调用令牌
 */
export async function get_weapp_access_token
(appid: string, _secret: string, force_refresh = false): Promise<WeappAccessToken>
{
	type Result = {
		access_token: string
		expires_in  : string
	...
	let { data } = await wxopen_api.post<Result>(
		...
		{ appid, secret: _secret, force_refresh, grant_type: 'client_credential' },
 ...
 * 小程序登录
 */
export async function get_weapp_session
(appid: string, _secret: string, js_code: string): Promise<WeappSession>
{
	type Params = {
		appid     : string
		secret    : string
	...
	let params: Params = { appid, secret: _secret, js_code, grant_type: 'authorization_code' }
 ...
 */
export async function get_weapp_unlimited
(
	access_token: string,
	page: string,
	scene: string,
 ...
 * 换取用户手机号
 */
export async function get_weapp_phone_number
 ...
 */
// eslint-disable-next-line max-params
export async function send_weapp_subscribe_message
(
	access_token: string,
	touser: string,
	template_id: string,
